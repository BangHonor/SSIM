...
function RemoveSection(value) {
	...
	for (; i < SectionNum; i++) {		//注意字符转数字
		Sections[i].name = Section[i + 1].name;
		Sections[i].group = Section[i + 1].group;
	}
...
function GeometryData() {
	...
	for (var i = 1; i < ElementNum; i++) {
		data.MEMBER[i - 1] = [i, Elements[i].section, Elements[i].node1, Elements[i].node2]
	}
	for (var i = 1; i < SectionNum; i++) {
		data.SECTION[i - 1] = [i,Sections[i].name,Sections[i].group]
	}
	for (var i = 1; i < MaterialNum; i++) {
		data.MATERIAL[i - 1] = [i, Materials[i].name, Materials[i].group]
	}
	return data;
}
