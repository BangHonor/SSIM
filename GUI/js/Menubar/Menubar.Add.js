...
Menubar.Add = function ( editor ) {
	...
	option.setTextContent( strings.getKey( 'menubar/add/new' ) );
...
//line
	...
	option.onClick( function () {
		...
		Swal.mixin({
			...
			if (result.value) {
				x1 = Number(result.value[0])
				y1 = Number(result.value[1])
				z1 = Number(result.value[2])
				x2 = Number(result.value[3])
				y2 = Number(result.value[4])
				z2 = Number(result.value[5])
			...
			else{Swal.fire("You didn't complete the operation.")}

		})
	} );
	/*
	var pAx = prompt("please enter point Ax","");
	var pAy = prompt("please enter point Ay","");
	var pAz = prompt("please enter point Az","");

	var pBx = prompt("please enter point Bx","");
	var pBy = prompt("please enter point By","");
	var pBz = prompt("please enter point Bz","");

	if(pAx && pAy && pAz && pBx && pBy && pBz ){
		x1 = Number(pAx);
		y1 = Number(pAy);
		z1 = Number(pAz);
		x2 = Number(pBx);
...
*/
	...
	option.onClick( function () {
		Swal.mixin({
			...
			if (result.value) {
				r = Number(result.value[0])
				var geometry = new THREE.SphereBufferGeometry(r, 8, 6, 0, Math.PI * 2, 0, Math.PI);
				var mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial());
				mesh.name = 'Sphere';
