...
var materialZ = new THREE.LineMaterial({
	...
	scene.add(lineX,lineY,lineZ);//line没有宽度!改用mesh
...
/*
function mesh() {
	var loaders = new THREE.FontLoader();
	var result;
	loaders.load("../GUI/Editor/examples/fonts/helvetiker_regular.typeface.json", function (font) {
...
function init() {
	...
	scene.add(planeTextX);
	planeTextX.position.set(0, 0, 1.1);

	planeTextY = PlaneText('Y', "#0000E3");
	scene.add(planeTextY);
	planeTextY.position.set(1.1, 0, 0);
	

	planeTextZ = PlaneText('Z', "#00DB00" );
	scene.add(planeTextZ);
	planeTextZ.position.set(0, 1.1, 0);
...
function animate() {
	...
	var scaleX=updateObj(planeTextX, 10);
	var scaleY=updateObj(planeTextY, 10);
	var scaleZ = updateObj(planeTextZ, 10);

	//同时更新文字的距离
	planeTextX.position.set(0, 0, 1.1 * scaleX);
	planeTextY.position.set(0, 1.1 * scaleY, 0);
	planeTextZ.position.set(1.1 * scaleZ, 0,0);
